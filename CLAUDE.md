...
**Name:** ogentic-adapter-sdk
**One-liner:** The Adapter protocol — the contract any LLM backend implements so ogentic-router can call it. Wave 2 OSS primitive.
**Owner:** David (CTO)
**Kind:** oss-library
...
- ruff + mypy strict
- Published to PyPI as `ogentic-adapter-sdk`
...
Once a method or class is exported from `ogentic_adapter_sdk.__init__`, it follows semver. Breaking changes go in a major bump and a deprecation notice ships first.

**Zero runtime dependencies on other Ogentic packages.**
This is the contract layer; it must be importable by downstream libs without pulling in shield/router/audit. Optional integrations live behind `pip install ogentic-adapter-sdk[router]` extras.
